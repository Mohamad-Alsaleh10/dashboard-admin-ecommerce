...
export default function Offer() {
    ...
    const handleDeleteCategory = async (categoryId) => {
        try {
            ...
            const response = await axios.delete(`https://api.alorfi-store.com/superAdmin_api/delete_offer?offerId=${categoryId}`, {
                headers: headers
            });
    ...
    useEffect(() => {
        ...
        const apiUrl = 'https://api.alorfi-store.com/superAdmin_api/show_offers';
                ...
                <table className="table">
                    <thead>
                        <tr className="bg-gray-2 text-left dark:bg-meta-4">
                            ...
                            <th className="py-4 px-4 font-medium text-black dark:text-white">offer_price</th>
                            <th className="py-4 px-4 font-medium text-black dark:text-white">from_date</th>
                            <th className="py-4 px-4 font-medium text-black dark:text-white">to_date</th>
                            <th className="py-4 px-4 font-medium text-black dark:text-white">currency</th>
                            <th className="py-4 px-4 font-medium text-black dark:text-white">like</th>
                    ...
                    {
                        responseData && responseData.map((offer, index) => {
                            return (

                                <tbody>

                                    <tr key={index} className="content-center">
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">{offer.id}</td>
                                        ...
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <span>
                                                {offer.currency == null ? 'Not found' : offer.currency}
                                            </span>
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <span>
                                        ...
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <img src={`https://api.alorfi-store.com/storage/${offer.image}`} alt="" />
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <button className="hover:text-primary " onClick={() => handleDeleteCategory(offer.id)}>
                                                <svg
