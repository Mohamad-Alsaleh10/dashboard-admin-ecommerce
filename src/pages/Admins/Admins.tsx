...
export default function Admins() {
  ...
  const handleClick = (customerId) => {
    // Construct the URL
    const apiUrl = `https://api.alorfi-store.com/superAdmin_api/active_or_not_admin?adminId=${customerId}`;
  ...
  const handledelete = async (categoryId) => {
    try {
      ...
      const response = await axios.delete(`https://api.alorfi-store.com/superAdmin_api/delete_admin?adminId=${categoryId}`, {
        headers: headers
      });
  ...
  useEffect(() => {
    ...
    const apiUrl = 'https://api.alorfi-store.com/superAdmin_api/show_admins';
  ...
  return (
    <DefaultLayout>
      {responseData && <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            ...
            <tbody>
              {responseData.map((packageItem, key) => (
                <tr key={key}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <button className="hover:text-primary pr-4" onClick={() => handledelete(packageItem.id)}>
                      <svg
                    ...
                    {packageItem.id > 1 &&                      <Link to={`/showadminpermission/${packageItem.id}`}>
                        <svg
                          className="fill-current"
                          width="18"
                          height="18"
